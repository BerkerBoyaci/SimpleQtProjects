...
void MainWindow::on_btnSave_clicked()
{
    ...
    QFileInfo fi(path);
    if(fi.exists()){
        QMessageBox::StandardButton btn = QMessageBox::question(this, "Exists", "File already exist. Would you like to overwrite it?");
        if(btn != QMessageBox::StandardButton::Yes)
            return;
    }

    if(!m_image.save(path, "PNG")){
        QMessageBox::critical(this, "Error" , "Could no save image");
